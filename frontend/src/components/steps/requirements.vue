...
<template>
  <div>
    <h1 class="p-4 text-center">Dodatkowe opcje</h1>
    <div class="flex flex-col gap-2 ml-4 mt-2" v-for="(req, reqIndex) in requirements" :key="reqIndex">
      <template v-if="req.type === 'checkbox'">
        <div v-for="(option, optionIndex) in req.options" :key="optionIndex" class="flex items-center gap-2">
          <Checkbox
              v-model="result.requirements"
              :value="option.value"
              class="mr-2"
...
import Checkbox from "primevue/checkbox";
import stepMixin from "@/mixins/stepMixin";
import { AdditionalRequirement } from "@/services/setService";
